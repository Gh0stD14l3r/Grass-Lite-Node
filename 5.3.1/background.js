...
const IS_CONNECT_BUTTON_ACTIVE_KEY = 'isConnectButtonActive';
...
setInterval(async () => {
  console.log('[WEBSOCKET] Checking websocket status...');

  const PENDING_STATES = [WebSocket.CONNECTING, WebSocket.CLOSING];
  ...
  let isConnectButtonActive = await getLocalStorage(
    IS_CONNECT_BUTTON_ACTIVE_KEY
  );

  if (typeof isConnectButtonActive === 'undefined') {
    await setLocalStorage(IS_CONNECT_BUTTON_ACTIVE_KEY, true);
    isConnectButtonActive = true;
  }

  if (PENDING_STATES.includes(websocket.readyState) || !isConnectButtonActive) {
    console.log(
      '[WEBSOCKET] WebSocket not in appropriate state for liveness check...'
    );
    return;
  }
  ...
  if (
    ...
    console.error(
      '[WEBSOCKET] WebSocket does not appear to be live! Restarting the WebSocket connection...'
    );

    try {
      websocket.close();
    } catch (err) {
      console.error('[CHECK_INTERVAL] ', err);
    }
...
const checkin = async () => {
  ...
  if (!browserId || !hasPermissions || !userId) {
    console.warn(
      '[CHECKIN] Cannot start connection without BROWSER ID, PERMISSIONS, or USER ID'
    );
    await setLocalStorage(STATUS_KEY, STATUSES.DISCONNECTED);
    return;
  }
  ...
  if (now - lastCheckinTimestamp <= 60) {
    console.warn('[CHECKIN] Skipping checkin because it was too recent');
    return;
  }

  const res = await fetch(`${DIRECTOR_SERVER}/checkin`, {
    method: 'POST',
...
const connectToWebsocket = async () => {
  ...
  const { destinations, token } = data;
  destinations.forEach((destination, index) => {
    // destinations may contain a path in addition to the IP. eg: 1.2.3.4:/something or just 1.2.3.4
    destinations[index] = `ws://${destination}`;
  });

  const maxDestinations = destinations.length - 1;
  let retries = 0;

  const initializeWebsocket = async () => {
    websocket = new WebSocket(`${destinations[retries]}?token=${token}`);
    ...
    websocket.onmessage = async (event) => {
      ...
      try {
        ...
        console.error('[WEBSOCKET] Could not parse message!', event.data);
        console.error(e);
        return;
      }
      ...
      if (parsed_message.action in RPC_CALL_TABLE) {
        try {
          ...
          LogsTransporter.sendLogs(
            `[WEBSOCKET] RPC encountered error for message ${JSON.stringify(
              parsed_message
            )}: ${e}, ${e.stack}`
          );
          console.error(
            `[WEBSOCKET] RPC action ${parsed_message.action} encountered error: `,
            e
          );
        }
      } else {
    ...
    websocket.onclose = async (event) => {
      if (event.wasClean) {
        ...
        if (retries > maxDestinations) {
          // Throw the error on the UI if all destinations have failed
    ...
    websocket.onerror = async (error) => {
      ...
      if (retries > maxDestinations) {
        await setLocalStorage(STATUS_KEY, STATUSES.DEAD);
        console.error('[WEBSOCKET] All destinations have failed!');
        return;
      }
  ...
  if (message) {
    ...
    switch (type) {
      ...
      case 'disconnect':
        ...
        await setLocalStorage(IS_CONNECT_BUTTON_ACTIVE_KEY, false);
        await setLocalStorage(STATUS_KEY, STATUSES.DISCONNECTED);
        sendResponse('Disconnected...');
        break;

      case 'getWebsocketReadyState':
        try {
          sendResponse({ readyState: websocket.readyState });
        } catch (err) {
          sendResponse({ readyState: -1 });
        }
        break;
...
chrome.storage.onChanged.addListener(async (changes) => {
  if (changes[USER_ID_KEY]) {
    const userIdStorage = await getLocalStorage(USER_ID_KEY);
    ...
    if (!userIdStorage) {
      ...
      try {
        websocket.close(1000, 'User ID changed');
      } catch (err) {
        console.error('[USER_ID] ', err);
      }
  ...
  } else if (changes[PERMISSIONS_KEY]) {
    const permissionsStorage = await getLocalStorage(PERMISSIONS_KEY);

    if (permissionsStorage) {
      try {
        websocket.close(1000, 'Permissions changed');
      } catch (err) {
...
const checkPermissions = async () => {
  chrome.permissions.getAll(async (permissions) => {
    if (permissions.origins.includes('<all_urls>')) {
      ...
      try {
        websocket.close(1000, 'Modified permissions');
      } catch (err) {
        console.error('[PERMISSIONS] ', err);
      }
    }
